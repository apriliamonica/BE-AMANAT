...
if (process.env.NODE_ENV !== "production") {
  try {
    ...
    console.warn(
      "Could not mount authRoutes from ./routes/authRoutes.js:",
      e.message
    );
  }
} else {
  ...
  try {
    if (process.env.NODE_ENV === "production")
      return res
        .status(403)
        .json({
          error: "Endpoint ini hanya tersedia di lingkungan development",
        });

    const { username } = req.body;
    if (!username)
      return res.status(400).json({ error: "Username harus disertakan" });
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      if (!username || !password)
        return res
          .status(400)
          .json({ error: "Username dan password harus disertakan" });
